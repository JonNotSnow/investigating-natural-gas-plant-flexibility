...
# n = 10000
for i in 1:n # Number of simulations

    println("-------------------------------------- This is $i-th simulation. --------------------------------------")

    ### Battery data ###
    charge_rate = floor(rand(Uniform(0.2, 1)) * max_demand / 1000) * 1000     # MW, max rate of charging/discharging
    battery_hour = rand([2; 4; 6; 8])                                         # Random storage hour rating
    battery_capacity = charge_rate * battery_hour                             # MWh
    efficiency = 0.9                                                          # Charge efficiency
    # efficiency_dis = round(1/efficiency, digits=1)                          # Discharge efficiency
    efficiency_dis = 0.9                                                      # Discharge efficiency

    ### Scale renewable generation
    total_pre_pene = rand(Uniform(0.2, 1.5))         # Total pre-penetration
    solar_pct = rand(Uniform())                      # % from solar
    wind_pct = 1 - solar_pct                         # % from wind

    # solar_scale = round(total_pre_pene * solar_pct / solar_pre_pene, digits=3)
    # wind_scale = round(total_pre_pene * wind_pct / wind_pre_pene, digits=3)
...
################################################################################
# Write to CSVs (Comment out if not in use)
################################################################################
